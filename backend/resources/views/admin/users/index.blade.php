<x-app-layout>
    ...
    <x-slot name="header">{{ __('pages.users') }}</x-slot>
    <x-slot name="create_btn">
        <a class="inline-flex items-center px-6 py-2 text-white bg-green-700 rounded-full hover:bg-green-800"
            ...
            <span class="text-white">{{ __('users.create_new_user') }}</span>
        </a>
    </x-slot>
            ...
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr
                        ...
                        <th class="px-4 py-3">{{ __('users.name') }}</th>
                        <th class="px-4 py-3">{{ __('users.email') }}</th>
                        <th class="px-4 py-3 text-center">{{ __('users.roles') }}</th>
                        ...
                        @foreach ($data as $key => $user)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-xs text-center">
                                    ...
                                    @else
                                        <label
                                            class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">{{ __('users.rolenot') }}</label>
                                    @endif
                                </td>
                                ...
                                <td class="text-sm font-medium leading-5 text-center whitespace-no-wrap">
                                    ...
                                    <a href="{{ route('users.show', ['user' => $user->id]) }}" class="text-indigo-600">
                                ...
                                <td class="text-sm font-medium leading-5 whitespace-no-wrap">
                                    <form action="{{ route('users.destroy', $user->id) }}" method="POST"
                                        onsubmit="return confirm('{{ trans('users.confirm') }}');">
